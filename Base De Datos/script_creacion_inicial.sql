...
--FALTA MIGRAR VIAJE PARA LUEGO ACTUALIZAR OPERACION)
	...
	SELECT  M.RESERVA_CODIGO,M.RESERVA_FECHA,M.PASAJE_CODIGO,M.PASAJE_FECHA_COMPRA,M.PASAJE_PRECIO,C.CLIE_ID
		,CA.CABINA_ID
		,V.VIAJE_ID
		FROM GD1C2019.gd_esquema.Maestra M
...
(@USUARIOID NVARCHAR(18), @USUARIOPASS NVARCHAR(255),
@retorno int output, @idUsuario bigint output ) AS
...
BEGIN
	...
	--declare @test varbinary (255) = convert(varbinary (255),@USUARIOPASS);
		declare @hash as varbinary(max) = HASHBYTES('SHA2_256', @USUARIOPASS )
		SELECT @estado=u.USER_ESTADO  FROM GD1C2019.ICE_CUBES.USUARIO U where @USUARIOID = u.USERID;
	--VERIFICA QUE EL USUARIO ESTE HABILITADO
		if (@estado =1)
		 BEGIN
		  SELECT @CONTADORUSU = COUNT(*) FROM GD1C2019.ICE_CUBES.USUARIO U  where @USUARIOID = u.USERID and u.USER_PASS = @hash;
	-- VERIFICA EL MATCH DEL USUARIO Y EL PASSWORD 
		  if (@CONTADORUSU = 0)
		   begin
	--NO ACERTÓ LA PASSWORD
			SELECT @cantidad_f = U.USER_INTENTOS FROM GD1C2019.ICE_CUBES.USUARIO U WHERE @USUARIOID = U.USERID;
			if (@cantidad_f = 2)
			 begin
	--NO TIENE MAS OPORTUNIDADES (SE BLOQUEA Y SE CUENTA 3)
			  UPDATE GD1C2019.ICE_CUBES.USUARIO SET USER_INTENTOS = 3, USER_ESTADO = 0 WHERE @USUARIOID = USERID;
			  set @retorno = 3;
			 end
			ELSE
			 BEGIN
	--ACUMULA UN ERROR FALLIDO
			  UPDATE GD1C2019.ICE_CUBES.USUARIO set USER_INTENTOS = (@cantidad_f + 1) WHERE @USUARIOID = USERID;
			  set @retorno = 1;
			 END
		   end
		  else
	--ACERTÓ. 
			  begin
			   UPDATE GD1C2019.ICE_CUBES.USUARIO  SET USER_INTENTOS = 0 WHERE @USUARIOID = USERID;
			   select @idUsuario= U.USERID from GD1C2019.ICE_CUBES.USUARIO U where U.USERID =@USUARIOID;
				BEGIN
				 set @retorno = 0; 
				END
			  END
