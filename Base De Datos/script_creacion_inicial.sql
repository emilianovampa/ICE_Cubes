...
/************************** INI-CREACION DE TABLAS***************************/
	...
	CREATE TABLE ICE_CUBES.USUARIO (
		USERDNI_ID nvarchar(18) PRIMARY KEY NOT NULL,
...
BEGIN
	...
	INSERT INTO ICE_CUBES.Usuario(USERDNI_ID, USER_TIPO,USER_PASS)
...
AS
BEGIN
/* Se dejan afuera de la migracion aquellos cliente que tienen DNI duplicado por el motivo
	que no se puede decidir cual es el correcto */
	...
	AND M.CLI_DNI NOT IN (SELECT  M.CLI_DNI
							FROM GD1C2019.gd_esquema.Maestra M
							WHERE M.CLI_DNI IS NOT NULL
							group by M.CLI_DNI
							having COUNT (distinct m.CLI_APELLIDO) > 1
							or COUNT (distinct m.CLI_FECHA_NAC) > 1 
							)
	order by 1 
END
