...
GO
 ...
 IF(OBJECT_ID('eliminarReservasVencidas') IS NOT NULL)
	DROP PROCEDURE eliminarReservasVencidas
GO
...
/************************** INI-CREACION DE TABLAS***************************/
	...
	--	COMP_SIMPLE INT )
	CREATE TABLE ICE_CUBES.MARCA(
		MARCA_ID INT IDENTITY(1,1) PRIMARY KEY  NOT NULL,
		MARCA_NOMBRE NVARCHAR(255), --FK MARCA
	)
...
-- Creacion de FKs
	...
	ALTER TABLE ICE_CUBES.CRUCERO WITH CHECK ADD
...
AS
BEGIN
	INSERT INTO ICE_CUBES.Funcionalidad (FUNCION_NOMBRE) VALUES ('Compra/Reserva de Viaje'); --1
	INSERT INTO ICE_CUBES.Funcionalidad (FUNCION_NOMBRE) VALUES ('Pago Reserva'); --2
	INSERT INTO ICE_CUBES.Funcionalidad (FUNCION_NOMBRE) VALUES ('ABM Puertos'); --3	
	INSERT INTO ICE_CUBES.Funcionalidad (FUNCION_NOMBRE) VALUES ('ABM Rol'); --4
	INSERT INTO ICE_CUBES.Funcionalidad (FUNCION_NOMBRE) VALUES ('ABM Usuarios'); --5
	INSERT INTO ICE_CUBES.Funcionalidad (FUNCION_NOMBRE) VALUES ('ABM Recorridos'); --6
	INSERT INTO ICE_CUBES.Funcionalidad (FUNCION_NOMBRE) VALUES ('ABM Cruceros'); --7
	INSERT INTO ICE_CUBES.Funcionalidad (FUNCION_NOMBRE) VALUES ('Generar Viaje'); --8
	INSERT INTO ICE_CUBES.Funcionalidad (FUNCION_NOMBRE) VALUES ('ListadosTOP'); --9
...
BEGIN
	...
	INSERT INTO ICE_CUBES.MARCA (MARCA_NOMBRE)
	SELECT DISTINCT M.CRU_FABRICANTE
	FROM GD1C2019.gd_esquema.Maestra M
	order by 1;

	INSERT INTO ICE_CUBES.CRUCERO (CRUCERO_IDENTIFICADOR,CRUCERO_MODELO,CRUCERO_MARCA_ID,CRUCERO_ESTADO)
	SELECT DISTINCT M.CRUCERO_IDENTIFICADOR,M.CRUCERO_MODELO, MC.MARCA_ID,1
	FROM GD1C2019.gd_esquema.Maestra M
	JOIN ICE_CUBES.MARCA MC ON MC.MARCA_NOMBRE = M.CRU_FABRICANTE;
	...
	SELECT distinct C.CRUCERO_ID,M.CABINA_NRO,M.CABINA_PISO,TC.TIPO_CABINA
	FROM GD1C2019.gd_esquema.Maestra M
	JOIN ICE_CUBES.TIPO_CABINA TC ON TC.TIPO_NOMBRE = M.CABINA_TIPO
	JOIN ICE_CUBES.CRUCERO C ON C.CRUCERO_IDENTIFICADOR =  m.CRUCERO_IDENTIFICADOR
	...
	INSERT INTO ICE_CUBES.VIAJE ( VIAJE_CRUCERO_ID,VIAJE_RECORRIDO_ID,VIAJE_FINICIO,VIAJE_FFIN)
		SELECT DISTINCT  C.CRUCERO_ID,M.RECORRIDO_CODIGO,M.FECHA_SALIDA,M.FECHA_LLEGADA
		FROM GD1C2019.gd_esquema.Maestra M
	    JOIN ICE_CUBES.CRUCERO C ON C.CRUCERO_IDENTIFICADOR =  m.CRUCERO_IDENTIFICADOR
		ORDER BY 3
...
--FALTA MIGRAR VIAJE PARA LUEGO ACTUALIZAR OPERACION)
	INSERT INTO ICE_CUBES.OPERACION(OPER_RESERVA_ID,OPER_F_RESERVA,OPER_PASAJE_ID,OPER_F_COMPRA
		,OPER_PRECIO,OPER_CLIENTE,OPER_CABINA_ID, OPER_VIAJE_ID,oper_estado)
	SELECT  M.RESERVA_CODIGO, M.RESERVA_FECHA, M.PASAJE_CODIGO,M.PASAJE_FECHA_COMPRA,M.PASAJE_PRECIO,C.CLIE_ID,CA.CABINA_ID,V.VIAJE_ID,1
		FROM GD1C2019.gd_esquema.Maestra M
		JOIN ICE_CUBES.CRUCERO CRU
			ON CRU.CRUCERO_IDENTIFICADOR = M.CRUCERO_IDENTIFICADOR
		LEFT JOIN ICE_CUBES.VIAJE V
			ON VIAJE_CRUCERO_ID = CRU.CRUCERO_ID
		...
		LEFT JOIN ICE_CUBES.CABINA CA
			ON CA.CABINA_CRUCERO_ID =CRU.CRUCERO_ID
...
END
GO
IF(OBJECT_ID('ICE_CUBES.SP_eliminarReservasVencidas') IS NOT NULL)
		DROP PROCEDURE ICE_CUBES.SP_eliminarReservasVencidas
GO
CREATE PROCEDURE ICE_CUBES.SP_eliminarReservasVencidas(@hoy DATE)
AS BEGIN
	BEGIN TRANSACTION
		update GD1C2019.ICE_CUBES.OPERACION
		set OPER_ESTADO = 0
		WHERE DATEADD(DAY, 3, OPER_F_RESERVA) <= @hoy
	COMMIT TRANSACTION
END
GO
IF(OBJECT_ID('ICE_CUBES.SP_BUSCARVIAJES') IS NOT NULL)
		DROP PROCEDURE ICE_CUBES.SP_BUSCARVIAJES
GO

CREATE PROCEDURE ICE_CUBES.SP_BUSCARVIAJES (@FDESDE DATE, @FHASTA DATE, @PORIGEN INT , @PDESTINO INT)
AS 
BEGIN
	SELECT V.VIAJE_ID,V.VIAJE_CRUCERO_ID,  V.VIAJE_RECORRIDO_ID, V.VIAJE_FINICIO, V.VIAJE_FFIN
	FROM GD1C2019.ICE_CUBES.VIAJE V
	JOIN GD1C2019.ICE_CUBES.RECORRIDO R ON R.RECO_ID = V.VIAJE_RECORRIDO_ID
	JOIN GD1C2019.ICE_CUBES.TRAMO T ON T.TRAMO_RECO_ID = R.RECO_ID
	where V.VIAJE_FINICIO >= @FDESDE
	AND V.VIAJE_FFIN <= @FHASTA
	AND R.RECO_ESTADO = 1
	AND T.TRAMO_ORIGEN = @PORIGEN
